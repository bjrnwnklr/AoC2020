    ...
    while all(players.values()):
        ...
        if current_config in configs:
            ...
            if all(len(players[p]) >= cards[p] for p in players):
                ...
                # check if we can skip the recursive game - if player 0 has the highest card in his
                # sub-deck, he will definitely win!
                max_card = max(c for deck in next_players.values() for c in deck)
                if max_card in next_players[0]:
                    winner = 0
                else:
